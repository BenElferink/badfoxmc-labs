...
const AirdropSnapshot = (props: {
  ...
  const runSnapshot = useCallback(async () => {
    ...
    try {
      ...
      const withHolders = !!holderPolicies.length
      ...
      if (withDelegators) {
        ...
        for await (const poolId of stakePools) {
          const poolDelegators: typeof delegators = []
          ...
          for (let page = 1; continueLoop; page++) {
            ...
            if (withHolders) {
              ...
              for await (const { stakeKey, delegatedLovelaces } of fetched.delegators) {
                if (!!delegatedLovelaces) {
                  setProgress((prev) => ({ ...prev, msg: `Processing Stake Pool Delegators (${poolDelegators.length})` }))
            ...
            if (!withBlacklist || (withBlacklist && !blacklistTokens.find((str) => str === tokenId))) {
              ...
              if (tokenAmount.onChain !== 0) {
                ...
                for (const owner of tokenOwners) {
                  ...
                  const isDelegator = !withDelegators || (withDelegators && !!delegators.find((obj) => obj.stakeKey === stakeKey))
      ...
      } else if (withDelegators) {
        for (const { delegatedLovelaces } of delegators) {
          divider += formatTokenAmount.fromChain(delegatedLovelaces, DECIMALS['ADA'])
        }
      }
      ...
      const payoutWallets = (
        withHolders
          ...
          ? delegators.map(({ stakeKey, address, delegatedLovelaces }) => {
              const amountForStake = formatTokenAmount.fromChain(delegatedLovelaces, DECIMALS['ADA']) * sharePerToken
      ...
      setTimeout(() => callback(payoutWallets), 0)
    } catch (error: any) {
      console.error(error)
