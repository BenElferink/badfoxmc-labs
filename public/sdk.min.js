class BadLabsSDK {
  constructor({ product: e, creatorStakeKey: t, otherStakeKeys: r }) {
    ;(this.product = e),
      (this.creatorStakeKey = t),
      (this.otherStakeKeys = r || ''),
  ...
  loadWallets = async (e) => {
    ...
    if (!['giveaways', 'polls'].includes(this.product)) throw Error('400 BAD REQUEST; "product" must be one of ["giveaways", "polls"]')
    ...
    if (n?.children.length) n.innerText = ''
    else {
      n.innerText = ''
      ...
      s.forEach((e) => {
        ...
        ;(s.style = `width: 150px; margin: 0.1rem; white-space: nowrap; text-transform: uppercase; color: ${i}; background-color: ${r}; border: none; border-radius: 0.5rem; display: flex; align-items: center; justify-content: flex-start;`),
          s.appendChild(t),
          s.appendChild(a),
          (s.onclick = async () => {
  ...
  start({ userStakeKey: e } = {}) {
    ...
    let t = `?product=${this.product}&creator_stake_key=${this.creatorStakeKey}&other_stake_keys=${this.otherStakeKeys}&user_stake_key=${e}`,
      r = this.frontUrl + t,
    ...
    ;(this.iFrameWrapper = document.createElement('div')),
      ...
      this.iFrame.setAttribute(
        'style',
        `max-width: ${i ? '100vw' : '800px'}; width: 100%; max-height: ${i ? '100vh' : '80vh'}; height: 100%; border: none; border-radius: ${
          i ? '0' : '1rem'
